...
auxSubjects = {"i":"a mi","you":"a ti","he":"a el", "she":"a ella",
       "they":"a ellos","we":"a nosotros"}
...
def tieneSentence(persona):
    if(persona=="yo"):
        return persona+" tengo"
    elif(persona=="tu"):
        return persona+" tienes "
    elif(persona=="el" or persona=="eso"):
        return persona+" tiene"
    elif(persona=="ella"):
        return persona+" tiene"
    elif(persona=="nosotros"):
        return persona+" tenemos"
    else:
        return persona+" tienen"
    
    
#Generar frases have
def generetaHaveSentences(objects,numbers):
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    words = objects.items()
    for inglesWord, spanishWord in words:
        ...
        subjectsItems = subjects.items()
        for inglesSubject, spanishSubject in subjectsItems:
            sentence = haveSentence(inglesSubject)
            fileIngles.write(sentence+" "+inglesWord+".\n")
            oracion = tieneSentence(spanishSubject)
            fileEspanol.write(oracion+" "+spanishWord+".\n") 
            numberWords = numbers.items()
            for number, numero in numberWords:
                fileIngles.write(sentence+" "+number+" "+inglesWord+".\n")
                fileEspanol.write(oracion+" "+numero+" "+spanishWord+".\n")
            
            
    fileIngles.close()
    fileEspanol.close()
    
#Generar frases de adjetivos
def genertaAdjectiveBeSentences(adjectivesBe):
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    words = adjectivesBe.items()
    for inglesWord, spanishWord in words:
        subjectsItems = subjects.items()
        for inglesSubject, spanishSubject in subjectsItems:
            sentence = subjectBeMethod(inglesSubject)
            sentence = sentence+" "+inglesWord
            fileIngles.write(sentence+".\n")
            oracion = personaBeMethod(spanishSubject,spanishWord)
            fileEspanol.write(oracion + ".\n")
            words2 = adjectivesBe.items()
            
            for inglesWord2, spanishWord2 in words2:
                andWord = andFunction(inglesWord2)
                sentence2 = sentence+andWord
                fileIngles.write(sentence2+".\n")
                yWord = personayMethod(spanishSubject,spanishWord2)
                fileEspanol.write(oracion+yWord+".\n")
            
            """words3 = adjectivesBe.items()
            
            for inglesWord3, spanishWord3 in words3:
                sentence3 = sentence+", "+inglesWord3+andWord
                fileIngles.write(sentence3+".\n")
                oracion3 = oracion+", "+spanishWord3+yWord
                fileEspanol.write(oracion3+".\n")"""
                
    fileIngles.close()
    fileEspanol.close()
    
def genertaAdjectiveNotBeSentences(adjectivesBe):
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    words = adjectivesBe.items()
    for inglesWord, spanishWord in words:
        ...
        for inglesSubject, spanishSubject in subjectsItems:
            sentence = subjectBeMethod(inglesSubject)
            sentence = sentence+" not "+inglesWord
            fileIngles.write(sentence+".\n")
            oracion = personaNotBeMethod(spanishSubject,spanishWord)
            fileEspanol.write(oracion + ".\n")
            words2 = adjectivesBe.items()
...
def generateAdjetiveSentences2(adjectives):
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    words = adjectives.items()
    for inglesWord, spanishWord in words:
        subjectsItems = subjects.items()
        for inglesSubject, spanishSubject in subjectsItems:
            sentence = subjectBeMethod(inglesSubject)
            sentence = sentence+" "+inglesWord
            fileIngles.write(sentence+".\n")
            oracion = personaMethod(spanishSubject,spanishWord)
            fileEspanol.write(oracion + ".\n")
            #words2 = adjectivesBe.items()
            words2 = adjectives.items()
...
def devolverSujetoHave(sujeto):
    if(sujeto == "yo"):
        return sujeto+" tengo "
    elif(sujeto == "tu"):
        return sujeto+" tienes "
    elif(sujeto == "el" or sujeto == "ella" or sujeto == "eso"):
        return sujeto+" tiene "
    elif(sujeto == "nosotros"):
        return sujeto+" tenemos "
    else:
        return sujeto+" tienen "
    
def devolverSujetoFruta(sujeto):
    if(sujeto == "a mi"):
        return sujeto+" me gusta "
    elif(sujeto == "a ti"):
        return sujeto+" te gusta "
    elif(sujeto == "a el" or sujeto == "a ella"):
        return sujeto+" le gusta "
    elif(sujeto == "a nosotros"):
        return sujeto+" nos gusta "
    else:
        return sujeto+" les gusta "
    
def articuloFuncion(palabra):
    if(palabra.endswith("a")):
        palabraRetornar = " la "+palabra
        return palabraRetornar
    else:
        palabraRetornar = " el "+palabra
        return palabraRetornar
        
def yFuncionFruta(palabra):
    palabraRetornar = " y "
    if(palabra.endswith("a")):
        palabraRetornar = palabraRetornar+" la "+palabra
        return palabraRetornar
    else:
        palabraRetornar = palabraRetornar+" el "+palabra
        return palabraRetornar
        
    

def generateLikeFruitSentences(arrayFruits):
    print("Hola si estoy entrando a la funcion")
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    ###
    ...
    for inglesWord, espanolWord in words:
        ...
        ##Escibir solo palabra
        fileIngles.write(inglesWord + ".\n")
        fileEspanol.write(espanolWord + ".\n")
        
        ##Escribir con un o una
        if(inglesWord.startswith(vocales)):
            fileIngles.write("an "+inglesWord + ".\n")
        else:
            fileIngles.write("a "+inglesWord + ".\n")
        
        if(espanolWord.endswith("a")):
            fileEspanol.write("una "+espanolWord+".\n")
        else:
            fileEspanol.write("un "+espanolWord+".\n")
        
        ##Escribir con los sujetos
        subjects = auxSubjects.items()
        for inglesSubject, espanolSubject in subjects:
            if(inglesSubject=="she" or inglesSubject=="he"):
                fileIngles.write(inglesSubject+" likes "+inglesWord+".\n")
            else:
                fileIngles.write(inglesSubject+" like "+inglesWord+".\n")
            ###Escribir con los sujetos y con y
            articulo = articuloFuncion(espanolWord)
            sujeto = devolverSujetoFruta(espanolSubject)
            fileEspanol.write(sujeto+articulo+".\n")
            words2 = arrayFruits.items()
            for fruit2,fruta2 in words2:
                andFruit = andFunction(fruit2)
                if(inglesSubject=="she" or inglesSubject=="he"):
                    fileIngles.write(inglesSubject+" likes "+inglesWord+andFruit+".\n")
                else:
                    fileIngles.write(inglesSubject+" like "+inglesWord+andFruit+".\n")
                
                yword = yFuncionFruta(fruta2)
                fileEspanol.write(sujeto+articulo+yword+".\n")
                """words3 = arrayFruits.items()
                for fruit3, fruta3 in words3:
                    articulo2 = articuloFuncion(fruta3)
                    if(inglesSubject=="she" or inglesSubject=="he"):
                        fileIngles.write(inglesSubject+" likes "+inglesWord+", "+fruit3+andFruit+".\n")
                    else:
                        fileIngles.write(inglesSubject+" like "+inglesWord+", "+fruit3+andFruit+".\n")
                    fileEspanol.write(sujeto+articulo+", "+articulo2+yword+".\n")"""
...
def generateNewHaveSentences(objects):
    fileIngles = open("ingles.txt","a+")
    fileEspanol = open("espanol.txt","a+")
    items = objects.items()
    for inglishWord, spanishWord in items:
        fileIngles.write(inglishWord+".\n")
        fileEspanol.write(spanishWord+".\n")
        
        auxA = "";
        ##Escribir con un o una
        if(inglishWord.startswith(vocales)):
            fileIngles.write("an "+inglishWord + ".\n")
            auxA = "an "+inglishWord
        else:
            fileIngles.write("a "+inglishWord + ".\n")
            auxA = "a "+inglishWord
        
        auxUn = ""
        if(spanishWord.endswith("a")):
            fileEspanol.write("una "+spanishWord+".\n")
            auxUn = "una "+spanishWord
        else:
            fileEspanol.write("un "+spanishWord+".\n")
            auxUn = "un "+spanishWord
            
        subjectsItems = subjects.items()
        for inglesSubject, spanishSubject in subjectsItems:
            if(inglesSubject=="she" or inglesSubject=="he" or inglesSubject == "it"):
                fileIngles.write(inglesSubject+" has "+auxA+".\n")
            else:
                fileIngles.write(inglesSubject+" have "+auxA+".\n")
            sujeto = devolverSujetoHave(spanishSubject)
            fileEspanol.write(sujeto+auxUn+".\n")
...
fruits = {"grape":"uva","lime": "lima", "lemon": "limón",
...
vegetables = {"carrot": "zanahoria","broccoli": "brócoli",
...
adjectivesBe = {"ugly":"feo","beautiful":"hermoso", "handsome" : "guapo",
                "lovely":"amado", "sweet" : "dulce", "disgusting" : "asqueroso",
                "black" : "negro","white" : "blanco", "red" : "rojo",
                "blue" : "azul", "yellow" : "amarillo", "orange" : "naranja",
                "green" : "verde", "purple" : "morado", "grey" : "gris",
                "brown" : "marrón", "clean" : "limpio", "tidy" : "ordenado",
                "dirty" : "sucio", "messy" : "desordenado", "fat" : "gordo",
                "thin" : "delgado", "tall" : "alto", "short" : "bajo",
                "big" : "grande", "small" : "pequeño", "boring" : "aburrido",
                "interesting" : "interesante", "slow" : "lento",
                "fast" : "rápido", "happy" : "feliz"}
...
#hiSentences(animals)
#genertaAdjectiveBeSentences(adjectivesBe)
#genertaAdjectiveNotBeSentences(adjectivesBe)
#generateAdjetiveSentences2(adjectives2)
generateLikeFruitSentences(fruits)
#generateLikeFruitSentences(vegetables)
#Merge(fruits, vegetables)
#generateLikeFruitSentences(vegetables)
#countSentences(numbers)
#generateGoingSentences(common100Verbs)
#generetaHaveSentences(objects,numbers)
#generateNewHaveSentences(animals)
#generateNewHaveSentences(fruits)
